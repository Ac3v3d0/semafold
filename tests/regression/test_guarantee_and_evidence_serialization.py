...
def test_guarantee_and_evidence_serialization_roundtrip() -> None:
    guarantee = CompressionGuarantee(
        ...
        bound_type="observed",
        value=0.01,
        workload_suitability=["reconstruction_only"],
    )
...
def test_codec_emitted_guarantees_and_evidence_remain_consumer_readable() -> None:
    ...
    assert passthrough_guarantees[0]["workload_suitability"] == ["embedding_storage", "reconstruction_only"]
    ...
    assert scalar_guarantees[0]["workload_suitability"] == ["embedding_storage", "reconstruction_only"]

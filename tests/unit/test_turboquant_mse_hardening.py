...
def test_turboquant_mse_handles_zero_rows() -> None:
    ...
    encoding = codec.encode(VectorEncodeRequest(data=data, objective=EncodeObjective.RECONSTRUCTION, seed=3))
...
def test_turboquant_mse_supports_additional_float_dtypes(dtype: np.dtype[np.generic]) -> None:
    ...
    encoding = codec.encode(VectorEncodeRequest(data=data, objective=EncodeObjective.RECONSTRUCTION, metric=EncodeMetric.MSE, seed=4))
    decoded = codec.decode(VectorDecodeRequest(encoding=encoding))
...
def test_turboquant_mse_decode_rejects_malformed_codebook_centers() -> None:
    ...
    metadata_segment = next(segment for segment in reloaded.segments if segment.segment_kind == EncodingSegmentKind.METADATA)
...
def test_turboquant_mse_decode_rejects_sidecar_size_mismatch() -> None:
    ...
    encoding = codec.encode(VectorEncodeRequest(data=data, objective=EncodeObjective.RECONSTRUCTION, seed=3))
    reloaded = VectorEncoding.from_dict(encoding.to_dict())
    sidecar_segment = next(segment for segment in reloaded.segments if segment.segment_kind == EncodingSegmentKind.SIDECAR)
